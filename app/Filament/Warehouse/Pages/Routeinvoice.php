...
class Routeinvoice extends Page implements  HasTable
{
    ...
    protected string $view = 'filament.warehouse.pages.routeinvoice';
     protected static bool $shouldRegisterNavigation = false;
    protected static ?string $navigationLabel = 'Route Invoice';
    protected static string | BackedEnum | null $navigationIcon = Heroicon::Briefcase;
    protected ?string $heading = 'Route Invoice';
    ...
    public function table(Table $table): Table
    {
        return $table
             ->headerActions([
                Action::make('Done')
                    ->url(fn($livewire) => WhdeliverylogResource::getUrl('edit', ['record' => $this->ownerRecord])),
               ...
               Action::make('addinvoice')
                   ->label('Add')
                    ...
                    ->action(function (Model $record) {
                        $assignedwh = Whdeliverylog::where('id', $this->ownerRecord)->first();
                   //     dd($assignedwh);

                        Whtripinvoice::create([
                 ...
                 BulkActionGroup::make([
                    ...
                    BulkAction::make('Add Delevery Invoice')
                        ...
                        ->action(function (Collection $records) {
                            $assignedwh = Whdeliverylog::where('id', $this->ownerRecord)->first();
                         //   dd($assignedwh->warehouse_id);
                            foreach ($records as $record) {
                                $checkinv = Whtripinvoice::where('invoice_id', $record->id)->first();
                                if (!$checkinv) {
